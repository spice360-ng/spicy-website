const G = "/group";

export const HERO_SLIDES = [
    `${G}/group-full.jpg`,
    `${G}/group-mgt.jpg`,
    `${G}/team-mixed-pose.jpg`,
    `${G}/team-female-discourse.jpg`,
    `${G}/team-mixed-huddle+devices-2.jpg`,
    `${G}/team-mixed-huddle+devices-3.jpg`,
    `${G}/team-mixed-casual.jpg`,
];

export const MOTTO_BG = `${G}/motto-mixed.jpg`;

export const PEOPLE_LANDING_BG = `${G}/group-full+overhead.jpg`;

export const CANNES_IMG = `${G}/team-mixed-discourse.jpg`;

export const FUN_PHOTOS = [
    `${G}/team-mixed-hifive+devices.jpg`,
    `${G}/team-hifive.jpg`,
    `${G}/goof-Gloria-Anavheokhai-Cinnamon.jpg`,
    `${G}/team-full-applause.jpg`,
    `${G}/team-mixed-casual.jpg`,
    `${G}/team-mixed-pose.jpg`,
    `${G}/goof-Adaobi-OnyenzeUgochukwu-Saffron.jpg`,
    `${G}/goof-Oluremi-Ogunwale-Crayfish.jpg`,
    `${G}/goof-Oladele-Gbolahan-spicename.jpg`,
    `${G}/group-full+overhead.jpg`,
];

export const OUR_STORY_SLIDES = [
    `${G}/team-mixed-huddle+devices-1.jpg`,
    `${G}/team-mixed-huddle+devices-2.jpg`,
    `${G}/team-mixed-huddle+devices-3.jpg`,
];

export const SPICE_ABOUT_BG = `${G}/team-mixed-discourse+devices.jpg`;
export const SPICE_AUDIENCE_BG = `${G}/team-mixed-hifive+devices.jpg`;

export const PEOPLE_FOOTER_WIDE = [
    `${G}/team-mixed-hifive+devices.jpg`,
    `${G}/team-hifive.jpg`,
    `${G}/team-full-applause.jpg`,
    `${G}/team-mixed-casual.jpg`,
    `${G}/team-mixed-pose.jpg`,
    `${G}/group-full+overhead.jpg`,
];

export const PEOPLE_FOOTER_MOBILE = [
    `${G}/goof-Gloria-Anavheokhai-Cinnamon.jpg`,
    `${G}/goof-Adaobi-OnyenzeUgochukwu-Saffron.jpg`,
    `${G}/goof-Oluremi-Ogunwale-Crayfish.jpg`,
    `${G}/goof-Oladele-Gbolahan-spicename.jpg`,
];
